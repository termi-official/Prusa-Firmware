...
void PID_autotune(float temp, int extruder, int ncycles) {
  ...
  for (;;) {
    ...
    if (pid_cycle > ncycles) {
      SERIAL_PROTOCOLLNPGM("PID Autotune finished! Put the last Kp, Ki and Kd "
                           "constants from above into Configuration.h");
      pid_tuning_finished = true;
      pid_cycle = 0;
      return;
    }
...
void manage_heater() {
  ...
  temp_runaway_check(0, target_temperature_bed, current_temperature_bed,
                     (int)soft_pwm_bed, true);
#endif

  for (int e = 0; e < EXTRUDERS; e++) {

#ifdef TEMP_RUNAWAY_EXTRUDER_HYSTERESIS
    temp_runaway_check(e + 1, target_temperature[e], current_temperature[e],
                       (int)soft_pwm[e], false);
#endif
...
#define K2 (1.0 - K1)
    ...
    if (fabs(current_temperature[0] - redundant_temperature) >
      ...
      if (IsStopped() == false) {
        SERIAL_ERROR_START;
        SERIAL_ERRORLNPGM("Extruder switched off. Temperature difference "
                          "between temp sensors is too high !");
...
// code for controlling the extruder rate based on the width sensor
  ...
  if (filament_sensor) {
    ...
    if (meas_shift_index < 0)
      meas_shift_index = meas_shift_index + (MAX_MEASUREMENT_DELAY +
                                             1); // loop around buffer if needed
...
// temperature offset adjustment
  ...
  if (celsius >= _offset_start && celsius <= _offset_center) {
    ...
    celsius = celsius + (_first_koef * (_offset_center - _offset_start)) +
...
#if (defined(TEMP_RUNAWAY_BED_HYSTERESIS) && TEMP_RUNAWAY_BED_TIMEOUT > 0) ||  \
    (defined(TEMP_RUNAWAY_EXTRUDER_HYSTERESIS) &&                              \
     TEMP_RUNAWAY_EXTRUDER_TIMEOUT > 0)
void temp_runaway_check(int _heater_id, float _target_temperature,
                        float _current_temperature, float _output,
                        bool _isbed) {
  float __hysteresis = 0;
  int __timeout = 0;
  bool temp_runaway_check_active = false;
  static float __preheat_start[2] = {0,
                                     0}; // currently just bed and one extruder
  ...
  if (millis() - temp_runaway_timer[_heater_id] > 2000) {
    ...
    if (temp_runaway_status[_heater_id] == TempRunaway_PREHEAT) {
      if (_current_temperature < ((_isbed) ? (0.8 * _target_temperature)
                                           : 150)) // check only in area where
                                                   // temperature is changing
                                                   // fastly for heater, check to
                                                   // 0.8 x target temperature
                                                   // for bed
      {
        __preheat_counter[_heater_id]++;
        // SERIAL_ECHOPGM("counter[0]:");
        // MYSERIAL.println(__preheat_counter[0]);  SERIAL_ECHOPGM("counter[1]:");
        // MYSERIAL.println(__preheat_counter[1]);  SERIAL_ECHOPGM("_isbed");
        // MYSERIAL.println(_isbed);
        if (__preheat_counter[_heater_id] >
            ((_isbed) ? 16
                      : 8)) // periodicaly check if current temperature changes
        {
    ...
    if (_current_temperature >= _target_temperature &&
        temp_runaway_status[_heater_id] == TempRunaway_PREHEAT) {
    ...
    if (temp_runaway_check_active) {
      //	we are in range
      if (_target_temperature - __hysteresis < _current_temperature &&
...
void temp_runaway_stop(bool isPreheat, bool isBed) {
  ...
  if (isPreheat) {
    Stop();
    isBed ? LCD_ALERTMESSAGEPGM("BED PREHEAT ERROR")
          : LCD_ALERTMESSAGEPGM("PREHEAT ERROR");
    SERIAL_ERROR_START;
    isBed ? SERIAL_ERRORLNPGM(" THERMAL RUNAWAY ( PREHEAT HEATBED)")
    ...
    isBed ? LCD_ALERTMESSAGEPGM("BED THERMAL RUNAWAY")
          : LCD_ALERTMESSAGEPGM("THERMAL RUNAWAY");
    SERIAL_ERROR_START;
    isBed ? SERIAL_ERRORLNPGM(" HEATBED THERMAL RUNAWAY")
...
void bed_min_temp_error(void) {
  ...
  if (IsStopped() == false) {
    SERIAL_ERROR_START;
    SERIAL_ERRORLNPGM(
        "Temperature heated bed switched off. MINTEMP triggered !");
...
#if defined(FILWIDTH_PIN) && (FILWIDTH_PIN > -1)
  static unsigned long raw_filwidth_value = 0; // added for filament width
                                               // sensor
#endif
...
#define MIN_STATE_TIME 16 // MIN_STATE_TIME * 65.5 = time in milliseconds
  ...
  case 9: // Measure FILWIDTH
    ...
    if (ADC > 102) // check that ADC is reading a voltage > 0.5 volts, otherwise
                   // don't take in the data.
    {
      raw_filwidth_value =
          raw_filwidth_value -
          (raw_filwidth_value >> 7); // multipliy raw_filwidth_value by 127/128
  ...
  case 10: // Startup, delay initial temp reading a tiny bit so the hardware can
           // settle.
    temp_state = 0;
    break;
    //    default:
